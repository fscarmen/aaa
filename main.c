...
static bool load_qr_library(QRLib *lib) {
    ...
    if (!lib->handle) {
        fprintf(stderr, "Error: libqrencode is required at runtime. Install libqrencode or build with it available.\n");
        return false;
    }
...
static void draw_compact(const unsigned char *matrix, int total_w, int total_h, int width, int qz, bool invert) {
    for (int y = 0; y < total_h; y += 2) {
        for (int x = 0; x < total_w; x++) {
            ...
            bool bottom = false;
            if (y + 1 < total_h) {
                bottom = cell(matrix, width, x, y + 1, qz, invert);
            }
...
static void draw_full(const unsigned char *matrix, int total_w, int total_h, int width, int qz, bool invert) {
    for (int y = 0; y < total_h; y++) {
        for (int x = 0; x < total_w; x++) {
            if (cell(matrix, width, x, y, qz, invert)) {
                fputs("██", stdout);
            } else {
                fputs("  ", stdout);
            }
...
static void draw_ascii_bitmap(const unsigned char *matrix, int width, int quiet_zone, bool compact, bool invert) {
    ...
    if (compact) {
        draw_compact(matrix, total_w, total_h, width, quiet_zone, invert);
    } else {
        draw_full(matrix, total_w, total_h, width, quiet_zone, invert);
    }
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(arg, "-level") == 0) {
            ...
            if (++i >= argc || !parse_int(argv[i], &quiet_zone)) {
                usage();
                return 1;
            }
        } else if (strncmp(arg, "-qz=", 4) == 0) {
            if (!parse_int(arg + 4, &quiet_zone)) {
                usage();
                return 1;
            }
