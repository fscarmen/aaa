...
- [`cfnat_macos.c`](cfnat_macos.c)：macOS 版本，参数面已与主线保持简化一致。
- [`cfnat_windows.c`](cfnat_windows.c)：Windows 7+ 版本，参数面同样已精简。
...
```bash
clang -O2 -pthread ./cfnat_macos.c -o ./cfnat-macos
```

说明：

- 适用于 amd64 和 arm64。
- macOS 版本当前保留原有单监听主链路，不包含 Linux 专属的百度前置代理与运营商分池实现。
...
如果需要调整，直接修改 [`cfnat_linux.c`](cfnat_linux.c) 顶部默认常量即可。
