...
如果 HTTP 响应能正常返回但没有 `CF-RAY`，或者 TCP 已经连接成功但当前网络环境没有读到 HTTP 响应，并且没有指定 `-colo`，程序会把这个 IP 作为 `UNK` 候选保留下来，再由后续健康检查确认是否可用。这样可以避免部分 Windows / 防火墙 / 运营商环境下因为响应头或读取行为差异导致候选池一直为空。
...
1. 开启 `-log=debug`，查看扫描统计里的连接失败、读取响应失败、缺少 `CF-RAY`、数据中心过滤数量。
