...
PROTOCOL_LIST=("VLESS + Reality Vision" "Hysteria2" "VLESS + Reality gRPC" "VLESS + WS" "VMess + WS" "Trojan + WS" "Shadowsocks + WS" "XHTTP + H2 CDN" "VLESS + XHTTP Direct" "Trojan Direct" "Shadowsocks 2022 Direct")
NODE_TAG=(     "reality-vision"         "hysteria2" "reality-grpc"         "vless-ws"   "vmess-ws"   "trojan-ws"   "ss-ws"            "xhttp-h2-cdn"   "xhttp-h3-direct"      "trojan-direct" "ss2022-direct")
...
# 协议标签辅助
TAG_REALITY_VISION="${NODE_TAG[0]}"
TAG_HYSTERIA2="${NODE_TAG[1]}"
TAG_REALITY_GRPC="${NODE_TAG[2]}"
TAG_VLESS_WS="${NODE_TAG[3]}"
TAG_VMESS_WS="${NODE_TAG[4]}"
TAG_TROJAN_WS="${NODE_TAG[5]}"
TAG_SS_WS="${NODE_TAG[6]}"
TAG_XHTTP_H2_CDN="${NODE_TAG[7]}"
TAG_XHTTP_H3_DIRECT="${NODE_TAG[8]}"
TAG_TROJAN_DIRECT="${NODE_TAG[9]}"
TAG_SS2022_DIRECT="${NODE_TAG[10]}"
...
E[100]=" a. all (default)"
C[100]=" a. 全部（默认）"
E[101]="XHTTP + H2 CDN (Temporary tunnel NOT supported)"
C[101]="XHTTP + H2 CDN（临时隧道不支持）"
...
fetch_nodes_value() {
  ...
  VLESS_XHTTP_PORT=$(echo "$JSON" | $WORK_DIR/jq -r '[.inbounds[] | select(.tag | split(" ")[-1] == "xhttp-h2-cdn") | .port] | .[0] // empty' 2>/dev/null)
  ...
  if [ -s "$WORK_DIR/inbound.json" ]; then
    ...
    while IFS=$'	' read -r TAG PORT; do
      ...
      case "$TAG" in
        ...
        vless-ws|vmess-ws|trojan-ws|ss-ws|xhttp-h2-cdn) [ "$HAS_NGINX" = false ] && append_unique_port EXPOSED_TCP_PORTS "$PORT" ;;
...
json_nginx() {
  ...
  if [ -s $WORK_DIR/inbound.json ] && [ -x $WORK_DIR/jq ]; then
    ...
    _PORT_XH=$(echo "$JSON_CLEAN" | $WORK_DIR/jq -r '[.inbounds[] | select(.tag | split(" ")[-1] == "xhttp-h2-cdn") | .port] | .[0] // empty' 2>/dev/null)
  fi
  ...
  grep -q 'xhttp-h2-cdn' <<< "$PROTOCOLS_NOW" && ! use_tunnel_direct_xhttp && _add_location "$(_xhttp_location "/${WS_PATH}-xh" "${_PORT_XH}")"
  local SUB_BLOCK
...
# 判断是否需要将 xhttp-h2-cdn 从 Nginx 下沉到 Tunnel 层
use_tunnel_direct_xhttp() {
  local _DOMAIN="${ARGO_DOMAIN}"
  [ -z "$_DOMAIN" ] && [ -s "$WORK_DIR/tunnel.yml" ] && _DOMAIN=$(awk '/^[[:space:]]*- hostname:/{print $3; exit}' "$WORK_DIR/tunnel.yml" 2>/dev/null)
  [ -z "$_DOMAIN" ] && [ -s "$WORK_DIR/list" ] && _DOMAIN=$(grep -m1 '^vless.*host=.*' "$WORK_DIR/list" | sed 's@.*host=\([^&]*\).*@\1@')
  [ -z "$_DOMAIN" ] && return 1
  [[ "$_DOMAIN" =~ trycloudflare\.com$ ]] && return 1
  get_installed_protocols | grep -qw 'xhttp-h2-cdn'
}

restart_argo_if_needed() {
...
' < "$WORK_DIR/tunnel.json")
  ...
  local _XHTTP_PORT
  [ -s "$WORK_DIR/inbound.json" ] && [ -x "$WORK_DIR/jq" ] && _XHTTP_PORT=$(grep -v '^//' "$WORK_DIR/inbound.json" | $WORK_DIR/jq -r '[.inbounds[] | select(.tag | split(" ")[-1] == "xhttp-h2-cdn") | .port] | .[0] // empty' 2>/dev/null)
  _XHTTP_PORT="${_XHTTP_PORT:-$VLESS_XHTTP_PORT}"
  [ -s "$WORK_DIR/inbound.json" ] && [ -x "$WORK_DIR/jq" ] && WS_PATH=$(grep -v '^//' "$WORK_DIR/inbound.json" | $WORK_DIR/jq -r '[.inbounds[] | select((.tag | split(" ")[-1]) == "xhttp-h2-cdn") | .streamSettings.xhttpSettings.path] | .[0] // empty' 2>/dev/null | sed 's|^/||; s|-xh$||')
  WS_PATH="${WS_PATH:-$WS_PATH_DEFAULT}"
  if [ -s "$WORK_DIR/inbound.json" ] && [ -x "$WORK_DIR/jq" ] && get_installed_protocols | grep -qw 'xhttp-h2-cdn'; then
    grep -v '^//' "$WORK_DIR/inbound.json"       | $WORK_DIR/jq --arg sn "$ARGO_DOMAIN" --arg cert "${WORK_DIR}/cert/cert.pem" --arg key "${WORK_DIR}/cert/private.key"         '.inbounds |= map(if ((.tag | split(" ")[-1]) == "xhttp-h2-cdn") then (.streamSettings.security = "tls" | .streamSettings.tlsSettings.serverName = $sn | .streamSettings.tlsSettings.certificates = [{"certificateFile":$cert,"keyFile":$key}]) else . end)'       > "$TEMP_DIR/inbound_tmp.json" && mv "$TEMP_DIR/inbound_tmp.json" "$WORK_DIR/inbound.json"
  fi
...
ingress:
EOF
  if use_tunnel_direct_xhttp && [ -n "$_XHTTP_PORT" ]; then
    cat >> $WORK_DIR/tunnel.yml << EOF
  - hostname: ${ARGO_DOMAIN}
    path: ^/${WS_PATH}-xh(?:/|$)
    service: https://127.0.0.1:${_XHTTP_PORT}
    originRequest:
      noTLSVerify: true
      http2Origin: true
      originServerName: ${ARGO_DOMAIN}
    ...
    INGRESS_JSON=$(cat <<EOF
        {
          "hostname": "${ARGO_DOMAIN}",
          "path": "^/${WS_PATH}-xh(?:/|$)",
          "service": "http://127.0.0.1:${_XHTTP_PORT}"
        },
        {
...
EOF
  ...
  local CONFIG_RESPONSE=$(wget --no-check-certificate -qO- --content-on-error   --method=PUT   --header="Authorization: Bearer ${CLOUDFLARE_API_TOKEN}"   --header="Content-Type: application/json"   --body-data="{
    "config": {
      "ingress": [
${INGRESS_JSON}
      ],
  ...
  for proto in "${INSTALL_PROTOCOLS[@]}"; do
    ...
    case "$proto" in
      b)
        BLOCK=$(cat << JSONEOF
    {
      "tag": "${NODE_NAME} ${TAG_REALITY_VISION}",
      "protocol": "vless",
      "port": ${REALITY_PORT},
      "settings": {
        "clients": [
...
JSONEOF
      ...
      d)
        BLOCK=$(cat << JSONEOF
    {
      "tag": "${NODE_NAME} ${TAG_REALITY_GRPC}",
      "protocol": "vless",
      "port": ${GRPC_PORT},
      "settings": {
        "clients": [
...
JSONEOF
      ...
      i)
        BLOCK=$(cat << JSONEOF
    {
      "tag": "${NODE_NAME} ${TAG_XHTTP_H2_CDN}",
      "protocol": "vless",
...
JSONEOF
      ...
      j)
        BLOCK=$(cat << JSONEOF
    {
      "tag": "${NODE_NAME} ${TAG_XHTTP_H3_DIRECT}",
      "port": ${XHTTP_PORT},
      "protocol": "vless",
      "settings": {
        "clients": [
...
JSONEOF
      ...
      k)
        BLOCK=$(cat << JSONEOF
    {
      "tag": "${NODE_NAME} ${TAG_TROJAN_DIRECT}",
      "protocol": "trojan",
      "port": ${TROJAN_PORT},
      "settings": {
        "clients": [
...
JSONEOF
      ...
      l)
        BLOCK=$(cat << JSONEOF
    {
      "tag": "${NODE_NAME} ${TAG_SS2022_DIRECT}",
      "protocol": "shadowsocks",
      "port": ${SS2022_PORT},
      "settings": {
...
export_list() {
  ...
  VMESS="{ \"v\": \"2\", \"ps\": \"${NODE_NAME} ${TAG_VMESS_WS}\", \"add\": \"${SERVER}\", \"port\": \"443\", \"id\": \"${UUID}\", \"aid\": \"0\", \"scy\": \"none\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"${ARGO_DOMAIN}\", \"path\": \"/${WS_PATH}-vm?ed=2560\", \"tls\": \"tls\", \"sni\": \"${ARGO_DOMAIN}\", \"alpn\": \"\" }"
  ...
  grep -q 'reality-vision' <<< "$PROTOS_NOW" && _add \
    "{name: \"${NODE_NAME} ${TAG_REALITY_VISION}\", type: vless, server: ${SERVER_IP}, port: ${REALITY_PORT}, uuid: ${UUID}, network: tcp, udp: true, tls: true, servername: ${TLS_SERVER}, flow: xtls-rprx-vision, client-fingerprint: chrome, reality-opts: {public-key: ${REALITY_PUBLIC}, short-id: \"\"} }" \
    "vless://$(echo -n "auto:${UUID}@${SERVER_IP_2}:${REALITY_PORT}" | base64 -w0)?remarks=${NODE_NAME// /%20}%20${TAG_REALITY_VISION}&obfs=none&tls=1&peer=${TLS_SERVER}&xtls=2&pbk=${REALITY_PUBLIC}" \
    "vless://${UUID}@${SERVER_IP_1}:${REALITY_PORT}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${TLS_SERVER}&fp=chrome&pbk=${REALITY_PUBLIC}&type=tcp&headerType=none#${NODE_NAME// /%20}%20${TAG_REALITY_VISION}" \
    "{ \"type\":\"vless\", \"tag\":\"${NODE_NAME} ${TAG_REALITY_VISION}\", \"server\":\"${SERVER_IP}\", \"server_port\": ${REALITY_PORT}, \"uuid\":\"${UUID}\", \"flow\":\"xtls-rprx-vision\", \"packet_encoding\":\"xudp\", \"tls\":{ \"enabled\":true, \"server_name\":\"${TLS_SERVER}\", \"utls\":{ \"enabled\":true, \"fingerprint\":\"chrome\" }, \"reality\":{ \"enabled\":true, \"public_key\":\"${REALITY_PUBLIC}\", \"short_id\":\"\" } } }" \
    "${NODE_NAME} ${TAG_REALITY_VISION}"
  ...
  if grep -q 'hysteria2' <<< "$PROTOS_NOW"; then
    ...
    _add \
      "{name: \"${NODE_NAME} ${TAG_HYSTERIA2}\", type: hysteria2, server: ${SERVER_IP}, port: ${HY2_PORT}, ${_chop}up: \"${_hy2_up} Mbps\", down: \"${_hy2_down} Mbps\", password: ${UUID}, sni: ${CERT_SNI}, skip-cert-verify: false, fingerprint: ${FP_SHA256}}" \
      "hysteria2://${UUID}@${SERVER_IP_1}:${HY2_PORT}?peer=${CERT_SNI}&hpkp=${FP_SHA256}&obfs=none&upmbps=${_hy2_up}&downmbps=${_hy2_down}${_srhop}${_h2h}#${NODE_NAME// /%20}%20${TAG_HYSTERIA2}" \
      "hy2://${UUID}@${SERVER_IP_1}:${HY2_PORT}?insecure=1&sni=${CERT_SNI}&upmbps=${_hy2_up}&downmbps=${_hy2_down}${_nekohop}${_h2h}#${NODE_NAME// /%20}%20${TAG_HYSTERIA2}" \
      "{ \"type\": \"hysteria2\", \"tag\": \"${NODE_NAME} ${TAG_HYSTERIA2}\", \"server\": \"${SERVER_IP}\", \"server_port\": ${HY2_PORT}${_sbhp}, \"up_mbps\": ${_hy2_up}, \"down_mbps\": ${_hy2_down}, \"password\": \"${UUID}\", \"tls\": { \"enabled\": true, \"server_name\": \"${CERT_SNI}\", \"certificate_public_key_sha256\": [\"${FP_BASE64}\"], \"alpn\": [ \"h3\" ] } }" \
      "${NODE_NAME} ${TAG_HYSTERIA2}"
  fi

  # reality-grpc
  grep -q 'reality-grpc' <<< "$PROTOS_NOW" && _add \
    "{name: \"${NODE_NAME} ${TAG_REALITY_GRPC}\", type: vless, server: ${SERVER_IP}, port: ${GRPC_PORT}, uuid: ${UUID}, network: grpc, udp: true, tls: true, servername: ${TLS_SERVER}, flow: , client-fingerprint: chrome, reality-opts: {public-key: ${REALITY_PUBLIC}, short-id: \"\"}, grpc-opts: {grpc-service-name: \"grpc\"} }" \
    "vless://$(echo -n "auto:${UUID}@${SERVER_IP_2}:${GRPC_PORT}" | base64 -w0)?remarks=${NODE_NAME// /%20}%20${TAG_REALITY_GRPC}&path=grpc&obfs=grpc&tls=1&peer=${TLS_SERVER}&pbk=${REALITY_PUBLIC}" \
    "vless://${UUID}@${SERVER_IP_1}:${GRPC_PORT}?security=reality&sni=${TLS_SERVER}&fp=chrome&pbk=${REALITY_PUBLIC}&type=grpc&serviceName=grpc&encryption=none#${NODE_NAME// /%20}%20${TAG_REALITY_GRPC}" \
    "{ \"type\": \"vless\", \"tag\":\"${NODE_NAME} ${TAG_REALITY_GRPC}\", \"server\": \"${SERVER_IP}\", \"server_port\": ${GRPC_PORT}, \"uuid\": \"${UUID}\", \"packet_encoding\":\"xudp\", \"tls\": { \"enabled\": true, \"server_name\": \"${TLS_SERVER}\", \"utls\": { \"enabled\": true, \"fingerprint\": \"chrome\" }, \"reality\": { \"enabled\": true, \"public_key\": \"${REALITY_PUBLIC}\", \"short_id\": \"\" } }, \"transport\": { \"type\": \"grpc\", \"service_name\": \"grpc\" } }" \
    "${NODE_NAME} ${TAG_REALITY_GRPC}"

  # vless-ws
  grep -q 'vless-ws' <<< "$PROTOS_NOW" && _add \
    "{name: \"${NODE_NAME} ${TAG_VLESS_WS}\", type: vless, server: ${SERVER}, port: ${SERVER_PORT_NOW}, uuid: ${UUID}, udp: true, tls: true, servername: ${ARGO_DOMAIN}, skip-cert-verify: false, network: ws, ws-opts: {path: \"/${WS_PATH}-vl\", headers: {Host: ${ARGO_DOMAIN}}, \"max_early_data\":2560, \"early_data_header_name\":\"Sec-WebSocket-Protocol\"} }" \
    "vless://${UUID}@${SERVER}:${SERVER_PORT_NOW}?encryption=none&security=tls&type=ws&host=${ARGO_DOMAIN}&path=/${WS_PATH}-vl?ed=2560&sni=${ARGO_DOMAIN}#${NODE_NAME// /%20}%20${TAG_VLESS_WS}" \
    "vless://${UUID}@${SERVER}:${SERVER_PORT_NOW}?encryption=none&security=tls&sni=${ARGO_DOMAIN}&type=ws&host=${ARGO_DOMAIN}&path=%2F${WS_PATH}-vl%3Fed%3D2560#${NODE_NAME// /%20}%20${TAG_VLESS_WS}" \
    "{ \"type\":\"vless\", \"tag\":\"${NODE_NAME} ${TAG_VLESS_WS}\", \"server\":\"${SERVER}\", \"server_port\":${SERVER_PORT_NOW}, \"uuid\":\"${UUID}\", \"tls\": { \"enabled\":true, \"server_name\":\"${ARGO_DOMAIN}\", \"utls\": { \"enabled\":true, \"fingerprint\":\"chrome\" } }, \"transport\": { \"type\":\"ws\", \"path\":\"/${WS_PATH}-vl\", \"headers\": { \"Host\": \"${ARGO_DOMAIN}\" }, \"max_early_data\":2560, \"early_data_header_name\":\"Sec-WebSocket-Protocol\" } }" \
    "${NODE_NAME} ${TAG_VLESS_WS}"

  # vmess-ws
  grep -q 'vmess-ws' <<< "$PROTOS_NOW" && _add \
    "{name: \"${NODE_NAME} ${TAG_VMESS_WS}\", type: vmess, server: ${SERVER}, port: ${SERVER_PORT_NOW}, uuid: ${UUID}, udp: true, alterId: 0, cipher: none, tls: true, servername: ${ARGO_DOMAIN}, skip-cert-verify: false, network: ws, ws-opts: {path: \"/${WS_PATH}-vm\", headers: {Host: ${ARGO_DOMAIN}}, \"max_early_data\":2560, \"early_data_header_name\":\"Sec-WebSocket-Protocol\"}}" \
    "vmess://$(echo -n "none:${UUID}@${SERVER}:${SERVER_PORT_NOW}" | base64 -w0)?remarks=${NODE_NAME// /%20}%20${TAG_VMESS_WS}&obfsParam=${ARGO_DOMAIN}&path=/${WS_PATH}-vm?ed=2560&obfs=websocket&tls=1&peer=${ARGO_DOMAIN}&alterId=0" \
    "vmess://$(echo -n "$VMESS" | base64 -w0)" \
    "{ \"type\":\"vmess\", \"tag\":\"${NODE_NAME} ${TAG_VMESS_WS}\", \"server\":\"${SERVER}\", \"server_port\":${SERVER_PORT_NOW}, \"uuid\":\"${UUID}\", \"tls\": { \"enabled\":true, \"server_name\":\"${ARGO_DOMAIN}\", \"utls\": { \"enabled\":true, \"fingerprint\":\"chrome\" } }, \"transport\": { \"type\":\"ws\", \"path\":\"/${WS_PATH}-vm\", \"headers\": { \"Host\": \"${ARGO_DOMAIN}\" }, \"max_early_data\":2560, \"early_data_header_name\":\"Sec-WebSocket-Protocol\" } }" \
    "${NODE_NAME} ${TAG_VMESS_WS}"

  # trojan-ws
  grep -q 'trojan-ws' <<< "$PROTOS_NOW" && _add \
    "{name: \"${NODE_NAME} ${TAG_TROJAN_WS}\", type: trojan, server: ${SERVER}, port: ${SERVER_PORT_NOW}, password: ${UUID}, udp: true, tls: true, servername: ${ARGO_DOMAIN}, sni: ${ARGO_DOMAIN}, skip-cert-verify: false, network: ws, ws-opts: {path: \"/${WS_PATH}-tr\", headers: {Host: ${ARGO_DOMAIN}}, \"max_early_data\":2560, \"early_data_header_name\":\"Sec-WebSocket-Protocol\" } }" \
    "trojan://${UUID}@${SERVER}:${SERVER_PORT_NOW}?peer=${ARGO_DOMAIN}&plugin=obfs-local;obfs=websocket;obfs-host=${ARGO_DOMAIN};obfs-uri=/${WS_PATH}-tr?ed=2560#${NODE_NAME// /%20}%20${TAG_TROJAN_WS}" \
    "trojan://${UUID}@${SERVER}:${SERVER_PORT_NOW}?security=tls&sni=${ARGO_DOMAIN}&type=ws&host=${ARGO_DOMAIN}&path=/${WS_PATH}-tr?ed%3D2560#${NODE_NAME// /%20}%20${TAG_TROJAN_WS}" \
    "{ \"type\":\"trojan\", \"tag\":\"${NODE_NAME} ${TAG_TROJAN_WS}\", \"server\": \"${SERVER}\", \"server_port\": ${SERVER_PORT_NOW}, \"password\": \"${UUID}\", \"tls\": { \"enabled\":true, \"server_name\":\"${ARGO_DOMAIN}\", \"utls\": { \"enabled\":true, \"fingerprint\":\"chrome\" } }, \"transport\": { \"type\":\"ws\", \"path\":\"/${WS_PATH}-tr\", \"headers\": { \"Host\": \"${ARGO_DOMAIN}\" }, \"max_early_data\":2560, \"early_data_header_name\":\"Sec-WebSocket-Protocol\" } }" \
    "${NODE_NAME} ${TAG_TROJAN_WS}"

  # ss-ws
  grep -qw 'ss-ws' <<< "$PROTOS_NOW" && _add \
    "{name: \"${NODE_NAME} ${TAG_SS_WS}\", type: ss, server: ${SERVER}, port: ${SERVER_PORT_NOW}, cipher: ${SS_METHOD}, password: ${UUID}, udp: true, plugin: v2ray-plugin, plugin-opts: { mode: websocket, host: ${ARGO_DOMAIN}, path: \"/${WS_PATH}-sh\", tls: true, servername: ${ARGO_DOMAIN}, skip-cert-verify: false, mux: false } }" \
    "ss://$(echo -n "chacha20-ietf-poly1305:${UUID}@${SERVER}:${SERVER_PORT_NOW}" | base64 -w0)?uot=2&v2ray-plugin=$(echo -n "{\"peer\":\"${ARGO_DOMAIN}\",\"mux\":false,\"path\":\"\\/${WS_PATH}-sh\",\"host\":\"${ARGO_DOMAIN}\",\"mode\":\"websocket\",\"tls\":true}" | base64 -w0)#${NODE_NAME// /%20}%20${TAG_SS_WS}" \
    "ss://$(echo -n "${SS_METHOD}:${UUID}" | base64 -w0)@${SERVER}:${SERVER_PORT_NOW}?plugin=v2ray-plugin%3Bmode%3Dwebsocket%3Bhost%3D${ARGO_DOMAIN}%3Bpath%3D%2F${WS_PATH}-sh%3Btls%3Dtrue%3Bservername%3D${ARGO_DOMAIN}%3Bskip-cert-verify%3Dfalse%3Bmux%3D0#${NODE_NAME// /%20}%20${TAG_SS_WS}" \
    "{ \"type\": \"shadowsocks\", \"tag\": \"${NODE_NAME} ${TAG_SS_WS}\", \"server\": \"${SERVER}\", \"server_port\": ${SERVER_PORT_NOW}, \"method\": \"chacha20-ietf-poly1305\", \"password\": \"${UUID}\", \"udp_over_tcp\": {\"enabled\": true,\"version\": 2}, \"plugin\": \"v2ray-plugin\", \"plugin_opts\": \"mode=websocket;host=${ARGO_DOMAIN};path=/${WS_PATH}-sh;tls=true;servername=${ARGO_DOMAIN};skip-cert-verify=false;mux=0\"}" \
    "${NODE_NAME} ${TAG_SS_WS}"

  # xhttp-h2-cdn (Standard XHTTP over Argo/CDN)，检查是否为临时隧道 (Try Tunnel)，临时隧道不支持 XHTTP + H2 CDN
  grep -q 'xhttp-h2-cdn' <<< "$PROTOS_NOW" && ! grep -q 'trycloudflare\.com$' <<< "${ARGO_DOMAIN}" && _add \
    "{name: \"${NODE_NAME} ${TAG_XHTTP_H2_CDN}\", type: vless, server: ${SERVER}, port: ${SERVER_PORT_NOW}, uuid: ${UUID}, udp: true, tls: true, network: xhttp, alpn: [h2], servername: ${ARGO_DOMAIN}, client-fingerprint: chrome, encryption: \"\", xhttp-opts: {path: \"/${WS_PATH}-xh\", host: ${ARGO_DOMAIN}} }" \
    "vless://$(echo -n ":${UUID}@${SERVER}:${SERVER_PORT_NOW}" | base64 -w0)?path=/${WS_PATH}-xh&remarks=${NODE_NAME// /%20}%20${TAG_XHTTP_H2_CDN}&obfsParam=%7B%22Host%22:%22${ARGO_DOMAIN}%22%7D&obfs=xhttp&tls=1&peer=${ARGO_DOMAIN}&alpn=h2,http/1.1&h2=1&mode=auto" \
    "vless://${UUID}@${SERVER}:${SERVER_PORT_NOW}?encryption=none&security=tls&sni=${ARGO_DOMAIN}&alpn=h2%2Chttp%2F1.1&type=xhttp&host=${ARGO_DOMAIN}&path=%2F${WS_PATH}-xh&mode=auto#${NODE_NAME// /%20}%20${TAG_XHTTP_H2_CDN}" \
    "" ""

  # xhttp-h3-direct
  grep -q 'xhttp-h3-direct' <<< "$PROTOS_NOW" && _add \
    "{name: \"${NODE_NAME} ${TAG_XHTTP_H3_DIRECT}\", type: vless, server: ${SERVER_IP}, port: ${XHTTP_PORT}, uuid: ${UUID}, udp: true, tls: true, network: xhttp, alpn: [h3], servername: ${CERT_SNI}, client-fingerprint: chrome, skip-cert-verify: false, fingerprint: ${FP_SHA256}, xhttp-opts: {path: \"/${WS_PATH}-xh3\", mode: stream-up} }" \
    "vless://$(echo -n \"auto:${UUID}@${SERVER_IP_1}:${XHTTP_PORT}\" | base64 -w0)?path=/${WS_PATH}-xh3&remarks=${NODE_NAME// /%20}%20${TAG_XHTTP_H3_DIRECT}&obfs=xhttp&tls=1&peer=${CERT_SNI}&alpn=h3&mode=stream-up&hpkp=${FP_SHA256}" \
    "vless://${UUID}@${SERVER_IP_1}:${XHTTP_PORT}?encryption=none&security=tls&sni=${CERT_SNI}&fp=chrome&alpn=h3&insecure=1&allowInsecure=1&pcs=${FP_SHA256//:/}&type=xhttp&path=%2F${WS_PATH}-xh3&mode=stream-up#${NODE_NAME// /%20}%20${TAG_XHTTP_H3_DIRECT}" \
    "" "${NODE_NAME} ${TAG_XHTTP_H3_DIRECT}"

  # trojan-direct
  grep -q 'trojan-direct' <<< "$PROTOS_NOW" && _add \
    "{name: \"${NODE_NAME} ${TAG_TROJAN_DIRECT}\", type: trojan, server: ${SERVER_IP}, port: ${TROJAN_PORT}, password: ${UUID}, udp: true, tls: true, sni: ${CERT_SNI}, servername: ${CERT_SNI}, skip-cert-verify: false, fingerprint: ${FP_SHA256} }" \
    "trojan://${UUID}@${SERVER_IP_1}:${TROJAN_PORT}?peer=${CERT_SNI}&tls=1&allowInsecure=0&sni=${CERT_SNI}&hpkp=${FP_SHA256}#${NODE_NAME// /%20}%20${TAG_TROJAN_DIRECT}" \
    "trojan://${UUID}@${SERVER_IP_1}:${TROJAN_PORT}?security=tls&sni=${CERT_SNI}&fp=chrome&allowInsecure=0&insecure=0&peer=${CERT_SNI}&pinSHA256=${FP_SHA256//:/}#${NODE_NAME// /%20}%20${TAG_TROJAN_DIRECT}" \
    "{ \"type\":\"trojan\", \"tag\":\"${NODE_NAME} ${TAG_TROJAN_DIRECT}\", \"server\": \"${SERVER_IP}\", \"server_port\": ${TROJAN_PORT}, \"password\": \"${UUID}\", \"tls\": { \"enabled\": true, \"server_name\": \"${CERT_SNI}\", \"certificate_public_key_sha256\": [\"${FP_BASE64}\"] } }" \
    "${NODE_NAME} ${TAG_TROJAN_DIRECT}"

  # ss2022-direct
  grep -q 'ss2022-direct' <<< "$PROTOS_NOW" && _add \
    "{name: \"${NODE_NAME} ${TAG_SS2022_DIRECT}\", type: ss, server: ${SERVER_IP}, port: ${SS2022_PORT}, cipher: 2022-blake3-aes-128-gcm, password: ${SS2022_PASSWORD}, udp: true }" \
    "ss://$(echo -n "2022-blake3-aes-128-gcm:${SS2022_PASSWORD}@${SERVER_IP_1}:${SS2022_PORT}" | base64 -w0)#$(echo -n "${NODE_NAME# }" | sed 's/ /%20/g')%20${TAG_SS2022_DIRECT}" \
    "ss://$(echo -n "2022-blake3-aes-128-gcm:${SS2022_PASSWORD}" | base64 -w0)@${SERVER_IP_1}:${SS2022_PORT}#${NODE_NAME// /%20}%20${TAG_SS2022_DIRECT}" \
    "{ \"type\": \"shadowsocks\", \"tag\": \"${NODE_NAME} ${TAG_SS2022_DIRECT}\", \"server\": \"${SERVER_IP}\", \"server_port\": ${SS2022_PORT}, \"method\": \"2022-blake3-aes-128-gcm\", \"password\": \"${SS2022_PASSWORD}\" }" \
    "${NODE_NAME} ${TAG_SS2022_DIRECT}"
...
change_protocols() {
  ...
  for tag in "${CURRENT_PROTOCOLS[@]}"; do
    for idx in "${!NODE_TAG[@]}"; do
      if [ "${NODE_TAG[$idx]}" = "$tag" ]; then
        ...
        [ "${NODE_TAG[$idx]}" = "xhttp-h2-cdn" ] && p_name=$(text 101)
  ...
  for idx in "${!NODE_TAG[@]}"; do
    ...
    for p in "${REINSTALL_PROTOCOLS[@]}"; do
      if [ "$p" = "$pname" ] || [ "$tag" = "xhttp-h2-cdn" -a "$p" = "$(text 101)" ] || [ "$tag" = "xhttp-h3-direct" -a "$p" = "VLESS + XHTTP Direct (h3)" ] || [ "$tag" = "trojan-direct" -a "$p" = "Trojan Direct" ] || [ "$tag" = "ss2022-direct" -a "$p" = "Shadowsocks 2022 Direct" ]; then
        REINSTALL_TAGS+=("$tag")
        break
      fi
    done
  ...
  for pname in "${REMOVE_PROTOCOLS[@]}"; do
    for idx in "${!PROTOCOL_LIST[@]}"; do
      [[ "${PROTOCOL_LIST[$idx]}" = "$pname" || ( "${NODE_TAG[$idx]}" = "xhttp-h2-cdn" && "$pname" = "$(text 101)" ) || ( "${NODE_TAG[$idx]}" = "xhttp-h3-direct" && "$pname" = "VLESS + XHTTP Direct (h3)" ) || ( "${NODE_TAG[$idx]}" = "trojan-direct" && "$pname" = "Trojan Direct" ) || ( "${NODE_TAG[$idx]}" = "ss2022-direct" && "$pname" = "Shadowsocks 2022 Direct" ) ]] && REMOVE_TAGS+=("${NODE_TAG[$idx]}") && break
    done
  done
  for pname in "${ADD_PROTOCOLS[@]}"; do
    for idx in "${!PROTOCOL_LIST[@]}"; do
      [[ "${PROTOCOL_LIST[$idx]}" = "$pname" || ( "${NODE_TAG[$idx]}" = "xhttp-h2-cdn" && "$pname" = "$(text 101)" ) || ( "${NODE_TAG[$idx]}" = "xhttp-h3-direct" && "$pname" = "VLESS + XHTTP Direct (h3)" ) || ( "${NODE_TAG[$idx]}" = "trojan-direct" && "$pname" = "Trojan Direct" ) || ( "${NODE_TAG[$idx]}" = "ss2022-direct" && "$pname" = "Shadowsocks 2022 Direct" ) ]] && ADD_TAGS+=("${NODE_TAG[$idx]}") && break
    done
  done
  ...
  for tag in "${REINSTALL_TAGS[@]}"; do
    ...
    if [ -n "$_EXIST_PORT" ]; then
      ...
      case "$tag" in
        ...
        xhttp-h2-cdn) VLESS_XHTTP_PORT=$_EXIST_PORT ;;
  ...
  for tag in "${REINSTALL_TAGS[@]}"; do
    ...
    if [ -z "$_EXIST_PORT" ]; then
      ...
      case "$tag" in
        ...
        xhttp-h2-cdn) VLESS_XHTTP_PORT=$_NEW_PORT ;;
  ...
  for _t in "${ADD_TAGS[@]}"; do
    [[ "$_t" =~ ^(vless-ws|vmess-ws|trojan-ws|ss-ws|xhttp-h2-cdn)$ ]] && _HAS_WS_XHTTP_ADD=true && break
  done
  ...
  for _t in "${REINSTALL_TAGS[@]}"; do
    [[ "$_t" =~ ^(vless-ws|vmess-ws|trojan-ws|ss-ws|xhttp-h2-cdn)$ ]] && _HAS_WS_XHTTP_FINAL=true && break
  done
  $_HAS_WS_XHTTP_FINAL || SERVER='__CDN_UNSET__'

  local _XHTTP_TLS_SERVER_NAME="$ARGO_DOMAIN"
  if printf '%s
' "${REINSTALL_TAGS[@]}" | grep -qx 'xhttp-h2-cdn'; then
...
EOF
  ...
  for tag in "${REINSTALL_TAGS[@]}"; do
    local NEW_BLOCK=''
    case "$tag" in
      hysteria2) NEW_BLOCK="{\"tag\":\"${NODE_NAME} ${TAG_HYSTERIA2}\",\"protocol\":\"hysteria\",\"port\":${HY2_PORT},\"settings\":{\"version\":2,\"clients\":[{\"auth\":\"${UUID}\"}]},\"streamSettings\":{\"network\":\"hysteria\",\"security\":\"tls\",\"tlsSettings\":{\"serverNames\":[\"${TLS_SERVER}\"],\"alpn\":[\"h3\"],\"certificates\":[{\"certificateFile\":\"${WORK_DIR}/cert/cert.pem\",\"keyFile\":\"${WORK_DIR}/cert/private.key\"}]}}}" ;;
      vless-ws) NEW_BLOCK="{\"port\":${VLESS_WS_PORT},\"listen\":\"127.0.0.1\",\"protocol\":\"vless\",\"tag\":\"${NODE_NAME} ${TAG_VLESS_WS}\",\"settings\":{\"clients\":[{\"id\":\"${UUID}\",\"level\":0}],\"decryption\":\"none\"},\"streamSettings\":{\"network\":\"ws\",\"security\":\"none\",\"wsSettings\":{\"path\":\"/${WS_PATH}-vl\"}},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\",\"quic\"],\"metadataOnly\":false}}" ;;
      vmess-ws) NEW_BLOCK="{\"port\":${VMESS_WS_PORT},\"listen\":\"127.0.0.1\",\"protocol\":\"vmess\",\"tag\":\"${NODE_NAME} ${TAG_VMESS_WS}\",\"settings\":{\"clients\":[{\"id\":\"${UUID}\",\"alterId\":0}]},\"streamSettings\":{\"network\":\"ws\",\"wsSettings\":{\"path\":\"/${WS_PATH}-vm\"}},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\",\"quic\"],\"metadataOnly\":false}}" ;;
      trojan-ws) NEW_BLOCK="{\"port\":${TROJAN_WS_PORT},\"listen\":\"127.0.0.1\",\"protocol\":\"trojan\",\"tag\":\"${NODE_NAME} ${TAG_TROJAN_WS}\",\"settings\":{\"clients\":[{\"password\":\"${UUID}\"}]},\"streamSettings\":{\"network\":\"ws\",\"security\":\"none\",\"wsSettings\":{\"path\":\"/${WS_PATH}-tr\"}},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\",\"quic\"],\"metadataOnly\":false}}" ;;
      ss-ws) NEW_BLOCK="{\"port\":${SS_WS_PORT},\"listen\":\"127.0.0.1\",\"protocol\":\"shadowsocks\",\"tag\":\"${NODE_NAME} ${TAG_SS_WS}\",\"settings\":{\"clients\":[{\"method\":\"chacha20-ietf-poly1305\",\"password\":\"${UUID}\"}],\"network\":\"tcp,udp\"},\"streamSettings\":{\"network\":\"ws\",\"wsSettings\":{\"path\":\"/${WS_PATH}-sh\"}},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\",\"quic\"],\"metadataOnly\":false}}" ;;
      xhttp-h2-cdn) NEW_BLOCK="{\"port\":${VLESS_XHTTP_PORT},\"listen\":\"127.0.0.1\",\"protocol\":\"vless\",\"tag\":\"${NODE_NAME} ${TAG_XHTTP_H2_CDN}\",\"settings\":{\"clients\":[{\"id\":\"${UUID}\",\"level\":0}],\"decryption\":\"none\"},\"streamSettings\":{\"network\":\"xhttp\",\"security\":\"tls\",\"tlsSettings\":{\"serverName\":\"${_XHTTP_TLS_SERVER_NAME}\",\"certificates\":[{\"certificateFile\":\"${WORK_DIR}/cert/cert.pem\",\"keyFile\":\"${WORK_DIR}/cert/private.key\"}]},\"xhttpSettings\":{\"path\":\"/${WS_PATH}-xh\",\"mode\":\"auto\"}}}" ;;
      xhttp-h3-direct) NEW_BLOCK="{\"tag\":\"${NODE_NAME} ${TAG_XHTTP_H3_DIRECT}\",\"port\":${XHTTP_PORT},\"protocol\":\"vless\",\"settings\":{\"clients\":[{\"id\":\"${UUID}\"}],\"decryption\":\"none\"},\"streamSettings\":{\"network\":\"xhttp\",\"security\":\"tls\",\"xhttpSettings\":{\"mode\":\"stream-up\",\"extra\":{\"alpn\":[\"h3\"]},\"path\":\"/${WS_PATH}-xh3\"},\"tlsSettings\":{\"serverName\":\"${TLS_SERVER}\",\"alpn\":[\"h3\"],\"certificates\":[{\"certificateFile\":\"${WORK_DIR}/cert/cert.pem\",\"keyFile\":\"${WORK_DIR}/cert/private.key\"}]}},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\",\"quic\"]}}" ;;
      trojan-direct) NEW_BLOCK="{\"port\":${TROJAN_PORT},\"protocol\":\"trojan\",\"tag\":\"${NODE_NAME} ${TAG_TROJAN_DIRECT}\",\"settings\":{\"clients\":[{\"password\":\"${UUID}\"}]},\"streamSettings\":{\"network\":\"tcp\",\"security\":\"tls\",\"tlsSettings\":{\"serverName\":\"${TLS_SERVER}\",\"certificates\":[{\"certificateFile\":\"${WORK_DIR}/cert/cert.pem\",\"keyFile\":\"${WORK_DIR}/cert/private.key\"}]}},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\",\"quic\"],\"metadataOnly\":false}}" ;;
      ss2022-direct) NEW_BLOCK="{\"port\":${SS2022_PORT},\"protocol\":\"shadowsocks\",\"tag\":\"${NODE_NAME} ${TAG_SS2022_DIRECT}\",\"settings\":{\"method\":\"2022-blake3-aes-128-gcm\",\"password\":\"${SS2022_PASSWORD}\",\"network\":\"tcp,udp\"},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\",\"quic\"],\"metadataOnly\":false}}" ;;
      reality-vision) NEW_BLOCK="{\"tag\":\"${NODE_NAME} ${TAG_REALITY_VISION}\",\"protocol\":\"vless\",\"port\":${REALITY_PORT},\"settings\":{\"clients\":[{\"id\":\"${UUID}\",\"flow\":\"xtls-rprx-vision\"}],\"decryption\":\"none\"},\"streamSettings\":{\"network\":\"tcp\",\"security\":\"reality\",\"realitySettings\":{\"show\":false,\"dest\":\"${TLS_SERVER}:443\",\"xver\":0,\"serverNames\":[\"${TLS_SERVER}\"],\"privateKey\":\"${REALITY_PRIVATE}\",\"publicKey\":\"${REALITY_PUBLIC}\",\"shortIds\":[\"\"]}},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\"]}}" ;;
      reality-grpc) NEW_BLOCK="{\"port\":${GRPC_PORT},\"protocol\":\"vless\",\"tag\":\"${NODE_NAME} ${TAG_REALITY_GRPC}\",\"settings\":{\"clients\":[{\"id\":\"${UUID}\",\"flow\":\"\"}],\"decryption\":\"none\"},\"streamSettings\":{\"network\":\"grpc\",\"security\":\"reality\",\"realitySettings\":{\"show\":false,\"dest\":\"${TLS_SERVER}:443\",\"xver\":0,\"serverNames\":[\"${TLS_SERVER}\"],\"privateKey\":\"${REALITY_PRIVATE}\",\"publicKey\":\"${REALITY_PUBLIC}\",\"shortIds\":[\"\"]},\"grpcSettings\":{\"serviceName\":\"grpc\",\"multiMode\":true}},\"sniffing\":{\"enabled\":true,\"destOverride\":[\"http\",\"tls\"]}}" ;;
    esac
...
change_argo() {
  ...
  # 若 Try 隧道且已安装 xhttp-h2-cdn，在类型后附加提示
  local ARGO_TYPE="$ARGO_TYPE"
  if [ "$ARGO_TYPE" = 'Try' ] && get_installed_protocols | grep -q 'xhttp-h2-cdn'; then
  ...
  cmd_systemctl enable argo
  if get_installed_protocols | grep -qw 'xhttp-h2-cdn' && [ -s "$WORK_DIR/inbound.json" ] && [ -x "$WORK_DIR/jq" ]; then
    unset ARGO_DOMAIN
    case $(grep "${DAEMON_RUN_PATTERN}" ${ARGO_DAEMON_FILE} 2>/dev/null) in
      *--config* ) fetch_tunnel_domain config >/dev/null 2>&1 || true ;;
      *--token* ) fetch_tunnel_domain config >/dev/null 2>&1 || true ;;
    ...
    [ -n "$ARGO_DOMAIN" ] && grep -v '^//' "$WORK_DIR/inbound.json"       | $WORK_DIR/jq --arg sn "$ARGO_DOMAIN" --arg cert "${WORK_DIR}/cert/cert.pem" --arg key "${WORK_DIR}/cert/private.key"         '.inbounds |= map(if ((.tag | split(" ")[-1]) == "xhttp-h2-cdn") then (.streamSettings.security = "tls" | .streamSettings.tlsSettings.serverName = $sn | .streamSettings.tlsSettings.certificates = [{"certificateFile":$cert,"keyFile":$key}]) else . end)'       > "$TEMP_DIR/inbound_tmp.json" && mv "$TEMP_DIR/inbound_tmp.json" "$WORK_DIR/inbound.json"
  fi
  export_list
}
...
change_start_port() {
  ...
  [ -s "$WORK_DIR/tunnel.json" ] && restart_argo_if_needed
  sleep 2
  export_list
  cmd_systemctl status xray &>/dev/null && info "
 Xray $(text 28) $(text 37) 
