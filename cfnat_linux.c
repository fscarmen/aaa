...
#define CACHE_TTL_SECONDS 86400L
...
static ResultList load_candidate_cache(const char *path) {
    ...
    if (fgets(line, sizeof(line), fp)) {
        if (sscanf(line, "# cfnat-cache-v1 %ld", &created_at) == 1 && created_at > 0) {
            long age = (long)time(NULL) - created_at;
            if (age > CACHE_TTL_SECONDS) {
                warn_msg("候选缓存 %s 已超过 24 小时，仍会先尝试快速健康检查", path);
            }
        } else {
            rewind(fp);
        }
